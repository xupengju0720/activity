	...
	JSDragDropTree.prototype = {
		...
		*/
		...
		dropDragableNodes:function()
		{
			...
			JSTreeObj.dragDropTimer = -1;	
			//dragNode_source  //拖拉对象
			//alert(JSTreeObj.dragNode_parent.id);   //拖拉对象的上级
		...
		deleteItem : function(obj1,obj2)
		{
			//obj1  点击项本身
			//obj2   被点击的标签
			var message = '点击确定删除节点(' + obj2.innerHTML+")";
		...
		initTree : function()
		{
			...
			if(this.deleteAllowed || this.renameAllowed){
				try{
					...
					if(this.renameAllowed)menuModel.addItem(2,'改名','','',false,'JSTreeObj.renameItem');
					if(this.addAllowed)menuModel.addItem(3,'创建','','',false,'JSTreeObj.addItem');
					menuModel.init();	
					
					var menuModelRenameOnly = new DHTMLGoodies_menuModel();
					if(this.renameAllowed)menuModelRenameOnly.addItem(4,'改名','','',false,'JSTreeObj.renameItem');
					menuModelRenameOnly.init();	
					
					var menuModelDeleteOnly = new DHTMLGoodies_menuModel();
					if(this.deleteAllowed)menuModelDeleteOnly.addItem(5,'删除','','',false,'JSTreeObj.deleteItem');
					menuModelDeleteOnly.init();	
					
					var menuModeladdOnly = new DHTMLGoodies_menuModel();
					if(this.addAllowed)menuModelRenameOnly.addItem(6,'创建','','',false,'JSTreeObj.addItem');
